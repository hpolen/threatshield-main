...
class Chat:
    def __init__(self, openai_handler):
        ...
        # keep client around if needed elsewhere, but we WON'T call .chat on it anymore
    ...
    def _truncate_context(self, context, max_chars=10000):
        ...
        if len(context) > max_chars:
            logging.warning(
                f'Context too long ({len(context)} chars), truncating to {max_chars} chars'
            )
    ...
    def _load_context(self, assessment_id):
        ...
        for name in [
            ...
            result = self.storage_handler.retrive_from_storage(
        ...
        structured_context = {
            "threat_model": self._extract_threat_model_summary(
                context_data.get("threat_model", {})
            ),
            "dread_assessment": self._extract_dread_summary(
                context_data.get("dread_assessment", {})
            ),
            "mitigations": self._extract_mitigation_summary(
                context_data.get("mitigation", {})
            ),
            "attack_tree": self._extract_attack_tree_summary(
                context_data.get("attack_tree", {})
            ),
    ...
    def _extract_threat_model_summary(self, threat_model_data):
        ...
        if isinstance(threat_model_data, dict):
            if (
                "raw_response" in threat_model_data
                and "threat_model" in threat_model_data["raw_response"]
            ):
    ...
    def _extract_dread_summary(self, dread_data):
        ...
        if table_data:
            try:
                table_data = sorted(
                    table_data,
                    key=lambda x: float(x.get("Risk Score", 0)),
                    reverse=True,
                )
    ...
    def _extract_mitigation_summary(self, mitigation_data):
        ...
        if isinstance(mitigation_data, dict):
            if "mitigations" in mitigation_data:
                ...
                "raw_response" in mitigation_data
                and "mitigations" in mitigation_data["raw_response"]
            ):
                mitigations = mitigation_data["raw_response"]["mitigations"]

        if mitigations:
            summary = "Mitigation Strategies Summary:\n"
            for i, item in enumerate(mitigations[:5], 1):
    ...
    def _extract_attack_tree_summary(self, attack_tree_data):
        ...
        if isinstance(attack_tree_data, dict):
            if "nodes" in attack_tree_data:
                ...
                "attack_tree" in attack_tree_data
                and "nodes" in attack_tree_data["attack_tree"]
            ):
        ...
        if nodes:
            ...
            for i, goal in enumerate(goals[:3], 1):
                ...
                if goal_id:
                    children = [
                        node for node in nodes if node.get("parent") == goal_id
                    ]
    ...
    def chat_about_report(self, assessment_id, user_query):
        try:
            ...
            if conversation_history:
                ...
                logging.info(
                    f'Using chat history with {len(conversation_history)} messages'
                )
            ...
            self._add_to_conversation_history(
                assessment_id, "assistant", response.get("Result", "")
            )
    ...
    def _process_query_with_context(self, user_query, context, history_str, assessment_id=None):
        ...
        try:
            ...
            # Special trigger: generate additional analyses via chat
            if assessment_id:
                threat_model_data = context.get("raw_data", {}).get("threat_model")

                if threat_model_data:
                    # Generate DREAD
                    if "generate" in lower_query and (
                        "dread" in lower_query or "risk assessment" in lower_query
                    ):
                        result = self.dread_handler.generate_dread_assessment(
                            threat_model_data
                        )
                        if result:
                            self.storage_handler.save_dread_assessment(
                                assessment_id, result["raw_response"]
                            )
                            if assessment_id in self.context:
                                self.context[assessment_id] = self._load_context(
                                    assessment_id
                                )
                            return {
                                "Result": (
                                    "I've generated a new DREAD assessment for you. "
                                    "Here's a summary of the top threats by risk score:\n\n"
                    ...
                    # Generate Mitigations
                        ...
                        # attack_tree_data and dread_data are optional here
                        result = self.mitigation_handler.generate_mitigations(
                            threat_model_data,
                            attack_tree_data=None,
                            dread_data=None,
                            assessment_id=assessment_id,
                        )
                        if result:
                            self.storage_handler.save_mitigation_result(
                            ...
                            return {
                                "Result": (
                                    "I've generated new mitigation strategies for you. "
                                    "Here's a summary:\n\n"
                    ...
                    # Generate Attack Tree
                        ...
                        result = self.attack_tree_handler.generate_attack_tree(
                            threat_model_data
                        )
                        if result:
                            self.storage_handler.save_attack_tree(
                                assessment_id, result["attack_tree"]
                            )
                            if assessment_id in self.context:
                                self.context[assessment_id] = self._load_context(
                                    assessment_id
                            ...
                            return {
                                "Result": (
                                    "I've generated a new attack tree for you. "
                                    "Here's a summary of the key attack paths:\n\n"
                                    + self._extract_attack_tree_summary(
                                        result["attack_tree"]
                                    )
                                )
                            }
...
"""
            ...
            if chat_prompt:
                system_context = chat_prompt.get(
                    "system_context",
                    "You are a threat modelling security expert. Answer questions in a conversational manner based on the provided context. Never return raw markdown tables or JSON directly - always provide natural language responses.",
                )
                task = chat_prompt.get(
                    "task", "Answer the user's questions based on the given context."
                )
...
6. Consider DREAD for risk assessment and deciding the severity of an issue. If asked about top issues or most important, consider DREAD to answer. Make sure to include all the Threats in DREAD before making a decision.
"""
                logging.warning(
                    "Chat prompt not found in prompts.json, using default prompt"
                )
                system_context = (
                    "You are a threat modelling security expert. Answer questions in a "
                    "conversational manner based on the provided context. Never return raw "
                    "markdown tables or JSON directly - always provide natural language responses."
                )

            # ------------------------------------------------------------
            # IMPORTANT CHANGE:
            # Use unified get_completion() instead of client.chat.completions
            # ------------------------------------------------------------
...
"""
            ...
            response_text = self.openai_handler.get_completion(
