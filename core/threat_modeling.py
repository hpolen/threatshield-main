...
class ThreatModelingHandler:
    def __init__(self, openai_handler):
        ...
        # We keep the client reference for backwards compatibility,
        # but all new calls should go through openai_handler.get_completion()
    ...
    def create_threat_model_prompt(
        ...
        logging.info(
            f"Custom prompt received in create_threat_model_prompt: '{custom_prompt[:100]}...'"
        )
        ...
        if assessment_id:
            ...
            error_msg = (
                "Assessment ID is required to determine the threat modeling methodology"
            )
...
"""
    ...
    def _get_methodology_from_details(self, assessment_id):
        ...
        try:
            with open(details_path, "r") as f:
                ...
                if not methodology:
                    error_msg = (
                        "No threat modeling methodology found in details.json "
    ...
    def json_to_markdown(
        self, threat_model, improvement_suggestions, assessment_id=None
    ):
        """
    ...
    def get_threat_model(self, prompt):
        ...
        IMPORTANT:
        - For OPENAI, we rely on OpenAIHandler.get_completion() which uses the Chat Completions API.
        - For BEDROCK, OpenAIHandler.get_completion() will delegate to BedrockHandler.get_completion().
        """
        method = self.openai_handler.method
        logging.info(f"Generating threat model using {method}")

        try:
            # System-style instructions to be prepended to the unified prompt
...
You MUST NOT include any explanatory text, markdown, or commentary outside the JSON object.
"""

            # Build a single text prompt for the unified get_completion() API
...
"""
            ...
            # Call the unified LLM handler (works for both OPENAI and BEDROCK)
            response_text = self.openai_handler.get_completion(
                full_prompt, max_tokens=8000
            )
            logging.info(f"Raw model response length: {len(response_text)}")
            logging.debug(
                f"Response content preview: {response_text[:200]}..."
                if len(response_text) > 200
                else f"Response content: {response_text}"
            )
            ...
            try:
                ...
                normalized_response = self._normalize_threat_model_response(
                    response_content
                )
                ...
                logging.error(f"Cleaned response was: {cleaned_text}")
    ...
    def _normalize_threat_model_response(self, response_content):
        ...
        for threat in threats:
            ...
            normalized_threat = {
                ...
                "Potential Impact": threat.get(
                    "Potential Impact", "Impact not specified"
                ),
            }
        ...
        if not isinstance(suggestions, list):
            logging.warning(
                "improvement_suggestions is not an array, attempting to normalize"
            )
    ...
    def generate_threat_model(
        ...
        logging.info(
            f"Custom prompt received in generate_threat_model: '{custom_prompt[:100]}...'"
        )
        ...
        markdown = self.json_to_markdown(
            threat_model, improvement_suggestions, assessment_id
        )
